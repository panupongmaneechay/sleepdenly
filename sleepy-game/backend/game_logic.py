...
def initialize_game():
    game_state = {
        ...
        "game_over": False,
        "winner": None,
        "theft_in_progress": None # { 'thief_player_id': 'playerX', 'thief_card_index': Y, 'target_player_id': 'playerZ', 'selected_card_indices_for_thief': [] }
    }
...
def apply_card_effect(game_state, playing_player_id, card_index, target_character_id=None, selected_card_indices_from_opponent=None, is_countering_theft=False, is_theft_cancellation=False):
    ...
    if is_theft_cancellation: # New flag for thief cancelling theft
        ...
        game_state["message"] = f"{game_state['players'][playing_player_id]['player_name']} cancelled the theft attempt."
        return game_state
    ...
    if is_countering_theft:
        ...
        if thief_card_index_on_thief_hand >= 0 and thief_card_index_on_thief_hand < len(game_state["players"][thief_player_id]["hand"]) and \
           game_state["players"][thief_player_id]["hand"][thief_card_index_on_thief_hand]["type"] == "theif":
            game_state["players"][thief_player_id]["hand"].pop(thief_card_index_on_thief_hand)
            game_state["message"] = f"{game_state['players'][playing_player_id]['player_name']} used {card['name']}! {game_state['players'][thief_player_id]['player_name']}'s Thief card was destroyed. Theft attempt nullified!"
        else:
            game_state["message"] = f"{game_state['players'][playing_player_id]['player_name']} used {card['name']}, but the Thief card was already gone or invalid. Theft attempt nullified!"
    ...
    if card["type"] == "theif":
        if selected_card_indices_from_opponent is None:
            ...
            game_state["message"] = f"{game_state['players'][playing_player_id]['player_name']} used Theif card. Waiting for opponent's response..."
    ...
    if target_character["current_sleep"] >= target_character["max_sleep"] and not target_character["is_asleep"]:
        ...
        if card["type"] not in ["lucky", "defensive", "dispel"]:
